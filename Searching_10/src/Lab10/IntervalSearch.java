...
public class IntervalSearch
{
	...
	Interval findInterval(T[] sortedData, TreeSet<T> targetValues)
	{
		...
		if (sortedData.length > 0 && targetValues.size() > 0)
		{
			...
			if(largestTarget.compareTo(sortedData[0]) < 0){
				leftBoundary = -1;
				rightBoundary = -1;
			}
			...
			while(first <= last && !found){
				...
				spot = mid;
				if(smallestTarget.compareTo(sortedData[mid]) == 0){
					spot = first;
					found = true;

				}
				else if(smallestTarget.compareTo(sortedData[mid]) < 0){
					last = mid - 1;
				...
				System.out.println("\tLeft: " + first + " Middle: " + mid + " Right: " + last);
			}
			boolean stop = false;
			for (int i = 0;!stop && found && i < sortedData.length; i++) {
				if(sortedData[i] == sortedData[mid]){
					spot = i;
					stop = true;
				}
			}
			...
			while(first <= last && !found){
				mid =  first + (last - first)/2;
				spot = mid;
				if(largestTarget.compareTo(sortedData[mid]) == 0){
					spot = last; //check
					found = true;
				}
				else if(largestTarget.compareTo(sortedData[mid]) < 0){
					last = mid - 1;
			...
			for (int i = 0;found && i < sortedData.length; i++) {
				if(sortedData[i] == sortedData[mid]){
					spot = i;
				}
			}
