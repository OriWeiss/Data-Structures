...
//else (last-first > a.length /8)
    ...
    void findNineCandidatePivotIndexes(ArrayList<Integer> indexes, T[] a, int first, int mid, int last, int size)
    {
        ...
        size = a.length/NUMBER_OF_SUB_ARRAYS;
        first=0;
        last = size;
        indexes.add(first);
        while(last < a.length){
            ...
            indexes.add(mid);
            findNineCandidatePivotIndexes(indexes, a,first+size, mid,  last+size, size);
        }
        indexes.add(last);
    ...
    void insertionSortNinePivotCandidates(ArrayList<Integer> indexes, T[] a)
    {
        int n = indexes.size();
        for (int j = 1; j < n; j++) {
            T key = a[indexes.get(j)];
            int i = j-1;
            while ( (i > -1) && ( a [i].compareTo(key)>0)) {
                a[indexes.get(i+1)] = a[i];
                i--;
            }
            a[indexes.get(i+1)] = key;
    ...
    int partition(T[] a, int first, int last)
    {
        ...
        else if (8 <= (last - first + 1) && (last - first + 1) <= 40) {
    ...
    public static void binaryRadixSort(Integer[] a, int maxNumber)
    {
        ...
        for (int i=0;i<a.length;i++) {
            if(a[i]%(int)Math.pow(10,i)==0){
                buckets[indexBucket0][i]= a[i];
            }
            else{
                buckets[indexBucket1][i]= a[i];
            }
            for (Integer[] array:buckets) {
