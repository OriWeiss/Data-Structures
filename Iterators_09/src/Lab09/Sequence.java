...
public class Sequence
{
    ...
    private boolean isSubSequenceOf(Sequence other)
    {
        ...
        int count = 0;
        while(other.sequence.size()!= 0 &&iterThis.hasNext()){//not supposed to be hasNext since when it doesnt have a next it means it's done and the reuslt is true
            itemOfThisList = iterThis.next();
            foundMatch = false;
            while(!foundMatch && iterOther.hasNext()){
